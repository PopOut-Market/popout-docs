...
This site ships Korean and English. Korean is the default locale, so the source
page goes in `docs/`. When you add a Korean page, add its English counterpart at
the same path under `i18n/en/docusaurus-plugin-content-docs/current/`. Relative
links resolve inside a locale tree, so a missing counterpart fails the English
build on any link pointing at that page.
...
Korean is the default locale, so `label` holds the Korean name. The English
category label goes in
`i18n/en/docusaurus-plugin-content-docs/current.json` instead.
