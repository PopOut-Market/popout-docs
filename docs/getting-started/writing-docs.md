...
이 사이트는 한국어와 영어를 제공합니다. 한국어가 기본 언어이므로 원문은 `docs/`에
씁니다. 한국어 페이지를 추가했다면 영어 대응 파일을
`i18n/en/docusaurus-plugin-content-docs/current/` 아래 같은 경로에 만드세요. 상대
...
한국어가 기본 언어이므로 `label`에는 한국어 이름을 씁니다. 영어 분류 이름은 이 파일이
아니라 `i18n/en/docusaurus-plugin-content-docs/current.json`에 들어갑니다.
